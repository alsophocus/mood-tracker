...
def add_mood_data():
    try:
        ...
        # Just insert mood data (user should already exist)
        mood_data = [
        ...
        for mood in mood_data:
            try:
                cursor.execute('''
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                ''', mood)
                print(f"✅ Added: {mood[1]} {mood[4][11:16]} - {mood[2]}")
            except Exception as e:
                print(f"⚠️ Skipped duplicate: {mood[1]} {mood[4][11:16]} - {mood[2]}")
        
        conn.commit()
        conn.close()
        
        print(f"\n🎉 Successfully added fake mood data to PostgreSQL!")
        print(f"📊 Added {len(mood_data)} mood entries for October 2025")
        
    except Exception as e:
        print(f"❌ Error adding mood data: {e}")
